...
## Use Case
A fixed camera monitors a fire exit corridor. When a person, bag, bin, or any other object enters and remains in front of the exit, safety regulations might be violated. This example processes individual JPEG frames from the camera feed and flags each distinct foreground object. The output includes a colorized label map as well as the original frame annotated with bounding boxes and object IDs. This makes it straightforward to integrate a downstream alerting or logging step.
...
- The w_source window is a Source window. Each incoming event contains one JPEG image frame. It is also the configured Score input window. When a request is received through the Scoring API, the image is injected here.
...
- The w_join window is a Join window. It performs a left-outer join between the connected component labeling (CCL) output and the original source frames on `index`, so that the annotation window has access to both the label-map metadata and the raw input image.
...
- `k`: The Sauvola sensitivity parameter. This determines how strict the thresholding algorithm is. Values closer to 0 allow more pixels to be classified as foreground objects but can allow more noise to be mistakenly classified. Values closer to 1 are more strict and only classify pixels with a strong local contrast as foreground, but may miss actual objects when set too high.
...
4. Notice lines 26 through 27:
    ...
    ```  
   This applies a threshold to remap every non-zero pixel to 255 and every zero pixel to 0. The incoming mask values (0 to 1) are scaled to the full 0 to 255 range, producing a high‑contrast black‑and‑white image that renders correctly as a JPEG.
...
- `xMinListOut`: List of coordinate values for the left edges of each bounding box.
- `xMaxListOut`: List of coordinate values for the right edges of each bounding box.
- `yMinListOut`: List of coordinate values for the top edges of each bounding box.
- `yMaxListOut`: List of coordinate values for the bottom edges of each bounding box.
...
This window joins the original frame from w_source with the CCL output from w_ccl so that the label map and the raw frame are both available in w_annotate.
...
4. Notice lines 22 through 23:
   ```
   ROI = (730, 10, 1230, 1050)
   ```
   The region of interest (for example, the fire exit area) and threshold (THRESH) for how much an object must overlap to be considered are defined.
5. Notice lines 65 through 68:
    ...
    ```
   These lines decode the single-channel CCL label map into an OpenCV image and handle cases where the output from the CCL window may appear as a three-channel image.
4. Notice lines 70 through 71:
    ```
    raw = esp_utils.image_conversion.blob_image_to_opencv_image(data["image"])
    raw = raw.copy()
    ```
   This decodes the original input frame.
5. Notice lines 87 through 91:
    ```
        if _bbox_inside_ratio(x1, y1, x2, y2, ROI) >= THRESH:
            ...
            ymaxs.append(y2)
    ```  
   The script calculates the overlap ratio of each bounding box against the fire exit's region of interest. Boxes that overlap by at least 30% are retained.
6. Notice lines 95 through 101:
    ...
    ```
   This code clears out any pixels from the label map that belong to objects outside of the region of interest.
...
#### Summary of the w_annotate window's main functions:
...
3. Bounding-box annotation: The filtered boxes are drawn with object ID labels onto the colorized label map and the original raw frame. A fixed random seed (42) ensures consistent box colors for each scoring API request notice line 20:
   ```
   np.random.seed(42)
   ```
...
To interact with the Scoring API, do the following:
1. Without selecting any of the windows, click <img src="img/properties_icon.png" alt="Project Properties icon" title="Project Properties Icon" width="20" /> in the right pane.
2. Expand **Scoring API**. Notice the following Scoring API-related attributes:
...
By default, any JPEG you submit is sent to the w_source window, routed through the image processing pipeline, and then the annotated `rawImage` from w_annotate is returned as the response.
...
One of the most powerful uses of the Scoring API is the ability to inspect the output of any window in the pipeline, not just the final result. This is particularly useful during development or parameter tuning. You can send the same image through the pipeline and see exactly what the pipeline produces at each stage without changing any other parts of the project.

To preview different stages, change the values of **Output window** and **Output field** in the **Scoring API** section before starting the project:

| Stage to preview                        | Output window        | Output field |
|-----------------------------------------|------------------------------|----------------------|
| After background subtraction            | `w_background_subtract`  | `subtracted_image`   |
| After grayscale conversion              | `w_gray`                 | `gray_image`         |
...
1. Without selecting a window, click <img src="img/properties_icon.png" alt="Project Properties icon" title="Project Properties Icon" width="20" /> in the right pane.
2. Expand **Scoring API**.
3. Change the value of **Output window** to `w_thresh_viz`.
4. Change the value of **Output field** to `viz_image`.
5. Select <img src="img/xml_editor_icon.png" alt="XML Editor Icon" title="XML Editor Icon" width="20" /> to open the XML editor and verify that the changes are reflected in `model.xml`:
```xml
<project ... score-output-window="cv_cq/w_thresh_viz" score-output-field="viz_image">
```

Follow the same steps in [Sending an Image and Receiving an Annotated Output](#sending-an-image-and-receiving-an-annotated-output). Because `viz_image` is an image blob, the **Save Output File** window appears with a preview of the binary mask. The preview shows pure white regions where foreground objects were detected and pure black everywhere else, making it easy to verify that the Sauvola binarization parameters are cleanly segmenting the foreground before connected component labeling runs.
...
To configure the project for structured output, remove the value of **Output field** before starting the project:
1. Without selecting a window, click <img src="img/properties_icon.png" alt="Project Properties icon" title="Project Properties Icon" width="20" /> in the right pane.
2. Expand **Scoring API**.
3. Change the value of **Output field** to `(none)`. The field should look like the figure below:  
   ![Empty score-output-field](img/empty_score_output_field.png "Empty score-output-field")
4. Click **Enter Test Mode**.
5. From the **Run Test** drop down, select **Configure and Run Test**.
6. In the **Load and Start Project in Cluster** window, adjust the deployment settings if necessary and click **OK**.
7. In the bottom pane, click the **Scoring API** tab.
8. In the **Input** table, click <img src="img/upload_input_file_icon.png" alt="Upload Input File Icon" title="Upload Input File Icon" width="30" />. The **Upload Input File** window appears.
9. In the test_files folder of the project package, select a test file (for example, `frame_person.jpg`).
10. Click **Send request**.
11. Notice that the response events appear in the **Output** table of the Scoring API tab, with one row per detected object that intersects the fire exit region of interest.
12. Click <img src="img/JSON_input.png" alt="JSON Icon" title="JSON" width="20" /> to view the events as JSON code:
<!-- the image in step 9 needs to be resized. I can barely see it -->
```json
[
  {
    "index": 1,
...
When the empty corridor (`static.jpg`) is used as the input instead of one of the obstruction frames, the w_fire_exit_filter window produces no qualifying events. The **Output** table is empty, confirming that no obstructions were detected.
...
4. Click **Enter Test Mode**.
5. In the left pane, select the windows whose output you want to examine. You should select **w_ccl** and **w_fire_exit_filter** because they display the outputs of object detection that can be previewed in the test mode table.
...
| ------------- | --------------- | ------------------- |
| `static.jpg` | © 2026 SAS Institute Inc. All Rights Reserved. | To be used only in the context of this demo. |
| `frame_bin.jpg` | © 2026 SAS Institute Inc. All Rights Reserved. | To be used only in the context of this demo. |
| `frame_bag.jpg` | © 2026 SAS Institute Inc. All Rights Reserved. | To be used only in the context of this demo. |
| `frame_person.jpg` | © 2026 SAS Institute Inc. All Rights Reserved. | To be used only in the context of this demo. |
